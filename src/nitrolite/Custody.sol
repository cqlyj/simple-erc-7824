...
import {IChannel} from "@nitrolite/contract/src/interfaces/IChannel.sol";
import {IDeposit} from "@nitrolite/contract/src/interfaces/IDeposit.sol";
import {IAdjudicator} from "@nitrolite/contract/src/interfaces/IAdjudicator.sol";
import {IComparable} from "@nitrolite/contract/src/interfaces/IComparable.sol";
import {Channel, State, Allocation, ChannelStatus, StateIntent, Signature, Amount} from "@nitrolite/contract/src/interfaces/Types.sol";
...
contract Custody is IChannel, IDeposit {
    ...
    function getAccountInfo(
        address user,
        address token
    ) public view returns (uint256 available, uint256 channelCount) {
    ...
    function create(
        ...
        if (_channels[channelId].stage != ChannelStatus.VOID)
        ...
        bool validSig = Utils.verifySignature(
            stateHash,
            initial.sigs[CLIENT],
            ch.participants[CLIENT]
        );
        ...
        if (initial.allocations.length != ch.participants.length)
    ...
    function join(
        ...
        bool allJoined = meta.actualDeposits[SERVER].amount ==
            meta.expectedDeposits[SERVER].amount;
    ...
    function close(
        ...
        if (meta.stage == ChannelStatus.ACTIVE) {
            ...
            if (!_verifyAllSignatures(meta.chan, candidate))
    ...
    function challenge(
        ...
        if (meta.stage == ChannelStatus.INITIAL && candidate.version != 0)
            revert InvalidState();
        ...
        if (candidate.data.length != 0) {
            if (candidate.intent == StateIntent.INITIALIZE) {
                ...
                uint256 deposited = meta.expectedDeposits[CLIENT].amount +
                    meta.expectedDeposits[SERVER].amount;
                uint256 expected = candidate.allocations[CLIENT].amount +
                    candidate.allocations[SERVER].amount;
        ...
        if (
            ...
            (candidate.intent != StateIntent.INITIALIZE &&
            ...
            bool isValid = IAdjudicator(meta.chan.adjudicator).adjudicate(
                meta.chan,
                candidate,
                proofs
            );
    ...
    function resize(
        ...
        if (candidate.version != precedingState.version + 1)
            revert InvalidState();
        ...
        _requireCorrectDelta(
            precedingState.allocations,
    ...
    function _requireCorrectAllocations(
        Allocation[] memory allocations
    ) internal pure {
        if (allocations.length != 2) revert InvalidState();
        if (allocations[CLIENT].token != allocations[SERVER].token)
            revert InvalidState();
    }
    ...
    function _unlockAllocation(
        ...
        uint256 correctedAmount = channelBalance > alloc.amount
            ? alloc.amount
            : channelBalance;
    ...
    function _verifyAllSignatures(
        ...
        bool isClientValid = Utils.verifySignature(
            stateHash,
            state.sigs[CLIENT],
            chan.participants[CLIENT]
        );
        if (!isClientValid) {
            return false;
        }
        ...
        bool isServerValid = Utils.verifySignature(
            stateHash,
            state.sigs[SERVER],
            chan.participants[SERVER]
        );
        if (!isServerValid) {
            return false;
        }
    ...
    function _isMoreRecent(
        ...
        try IComparable(adjudicator).compare(candidate, previous) returns (
            int8 result
        ) {
    ...
    function _requireCorrectDelta(
        ...
        uint256 sumBefore = initialAllocations[CLIENT].amount +
            initialAllocations[SERVER].amount;
        int256 sumDelta = delta[CLIENT] + delta[SERVER];
        uint256 sumAfter = finalAllocations[CLIENT].amount +
            finalAllocations[SERVER].amount;
    ...
    function _processResize(
        ...
        for (uint256 i = 0; i < 2; i++) {
            if (resizeAmounts[i] > 0) {
                _lockAccountFundsToChannel(
                    chMeta.chan.participants[i],
                    channelId,
                    token,
                    uint256(resizeAmounts[i])
                );
            }
        }
