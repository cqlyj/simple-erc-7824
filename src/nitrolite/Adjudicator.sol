...
import {Channel, State, Allocation, Signature, Amount, StateIntent} from "@nitrolite/contract/src/interfaces/Types.sol";
...
contract Adjudicator is IAdjudicator, IComparable {
    ...
    function adjudicate(
        ...
        Remittance memory candidateRemittance = abi.decode(
            candidate.data,
            (Remittance)
        );
        ...
        if (earliestProof.intent == StateIntent.INITIALIZE) {
            ...
            uint8 otherParty = candidateRemittance.sender == CREATOR
                ? BROKER
                : CREATOR;
    ...
    function _validateRemittanceState(
        Channel calldata chan,
        State memory state
        ...
        Remittance memory currentRemittance = abi.decode(
            state.data,
            (Remittance)
        );
        ...
        return
            Utils.verifySignature(
                stateHash,
                state.sigs[0],
                chan.participants[currentRemittance.sender]
            );
    }

    function _validateRemittanceTransition(
        State memory previousState,
        ...
        Remittance memory previousRemittance = abi.decode(
            previousState.data,
            (Remittance)
        );
        Remittance memory currentRemittance = abi.decode(
        ...
        if (
            currentState.allocations[0].token !=
            previousState.allocations[0].token
        ) {
            return false;
        }

        return
            _validateRemittanceIsApplied(
                currentRemittance,
    ...
    function _validateRemittanceIsApplied(
        ...
        if (
            prevAllocations[remittance.sender].amount <=
    ...
    ) external pure returns (int8 result) {
