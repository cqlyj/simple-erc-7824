...
import {INitroAdjudicator} from "@nitro/src/interfaces/INitroAdjudicator.sol";
...
contract NitroAdjudicator is INitroAdjudicator, ForceMove, MultiAssetHolder {
    ...
    function transferAllAssets(
        ...
        _requireMatchingFingerprint(
            stateHash,
            NitroUtils.hashOutcome(outcome),
            channelId
        );

        // computation
        bool allocatesOnlyZerosForAllAssets = true;
        Outcome.SingleAssetExit[] memory exit = new Outcome.SingleAssetExit[](
            outcome.length
        );
