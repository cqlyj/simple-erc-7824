...
import {IMultiAssetHolder} from "@nitro/src/interfaces/IMultiAssetHolder.sol";
...
contract MultiAssetHolder is IMultiAssetHolder, StatusManager {
    ...
    function deposit(
        ...
        require(
            !_isExternalDestination(channelId),
            "Deposit to external destination"
        );
        ...
        )
    {
        ...
        // target checks
        require(
            targetOutcome[targetAssetIndex].asset == asset,
    ...
    function compute_reclaim_effects(
        ...
        Outcome.Allocation memory guarantee = sourceAllocations[
            indexOfTargetInSource
        ];
        Guarantee memory guaranteeData = decodeGuaranteeData(
            guarantee.metadata
        );
        ...
        for (uint256 i = 0; i < sourceAllocations.length; i++) {
            ...
            if (
                !foundLeft &&
                sourceAllocations[i].destination == guaranteeData.left
            ) {
            ...
            if (
                !foundRight &&
                sourceAllocations[i].destination == guaranteeData.right
            ) {
        ...
        require(
            totalReclaimed == guarantee.amount,
            "totalReclaimed!=guarantee.amount"
        );
     ...
     */
    function _executeSingleAssetExit(
        Outcome.SingleAssetExit memory singleAssetExit
    ) internal {
     ...
     */
    function _isExternalDestination(
        bytes32 destination
    ...
    function _requireChannelFinalized(bytes32 channelId) internal view {
        require(
            _mode(channelId) == ChannelMode.Finalized,
    ...
    function decodeGuaranteeData(
        bytes memory data
    ) internal pure returns (Guarantee memory) {
