...
import {IForceMove} from "@nitro/src/interfaces/IForceMove.sol";
...
contract ForceMove is IForceMove, StatusManager {
    ...
    function challenge(
        ...
        bytes32 supportedStateHash = NitroUtils.hashState(
            fixedPart,
    ...
    function _conclude(
        ...
        RecoveredVariablePart
            memory recoveredVariablePart = recoverVariablePart(
                fixedPart,
                candidate
            );
        ...
        // We only need one signature to finalize the channel
        require(
            NitroUtils.getClaimedSignersNum(recoveredVariablePart.signedBy) ==
                1,
            "!unanimous"
    ...
    function _requireChallengerIsParticipant(
        ...
        require(
            _isAddressInArray(challenger, participants),
    ...
    function recoverVariableParts(
        ...
        RecoveredVariablePart[]
            memory recoveredVariableParts = new RecoveredVariablePart[](
                signedVariableParts.length
            );
        for (uint256 i = 0; i < signedVariableParts.length; i++) {
            recoveredVariableParts[i] = recoverVariablePart(
                fixedPart,
                signedVariableParts[i]
            );
        }
        ...
        for (uint256 j = 0; j < signedVariablePart.sigs.length; j++) {
            address signer = NitroUtils.recoverSigner(
                NitroUtils.hashState(
                    fixedPart,
                    signedVariablePart.variablePart
                ),
    ...
    function _requireIncreasedTurnNumber(
        ...
        require(
            newTurnNumRecord > turnNumRecord,
            "turnNumRecord not increased."
        );
    }
     ...
     */
    function _matchesStatus(
        ChannelData memory data,
        bytes32 s
    ) internal pure returns (bool) {
